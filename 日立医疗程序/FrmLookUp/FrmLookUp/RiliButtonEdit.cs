...
namespace FrmLookUp
{
    ...
    public partial class RiliButtonEdit : UserControl, ILookupClear
    {
        ...
        public FrmLookUp.RiliFrmLookUpBase LookForm;
       // public FrmLookUp.FrmLookUpBase LookForm = new FrmLookUpBase();
        ...
        private void buttonEdit2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (LookForm == null)
                {
                    LookForm = new FrmLookUp.RiliFrmLookUpBase();
                    LookForm.btn = this;
                }
                if (BaoSQL != "")
                {
                ...
                else if (ShowTable != null)
                {
                    LookForm.Init(ShowTable);
                }
                else
                {
        ...
        private void ButtonEdit_Leave(object sender, EventArgs e)//当失去焦点的时候发生，可以检验
        {
            ...
            if (DecideSql != "")
            {
                ...
                ((DataAccess.RiliLookUp)objLookUp).SQL = DecideSql+"'"+this.Text.ToString()+"'";
                DataTable tables = objLookUp.GetInfo();
                if (tables.Rows.Count < 1)
                {

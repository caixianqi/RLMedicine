    ...
    public partial class Form1 : Bao.FormChildBase, Bao.Interface.IU8Contral
    {
        ...
        { // 状态cState：新任务、已派遣、待审核、已审核
            ...
            { //工程师 cCallManger 
                ...
                //    button2.Text = "提交";
                //}
                else if (DTCallBack.Rows[0]["cState"].ToString() == "待审核")//回访人姓名
                {
                    BtnModify.Enabled = false;
                    button2.Enabled = true;
                    button2.Text = "收回";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (DTCallBack.Rows[0]["cState"].ToString() == "待审核")
                {
                    try
                    {
                        string sql = "update CallBack set cAuditMessageId=null,cState = '已派遣' where cTaskCode='" + tbTaskCode.Text + "'";
                        RiLiGlobal.RiLiDataAcc.ExecuteNotQuery(sql);
        ...
        {
            tbRegion.BaoSQL = "select cDCCODE,CDCNAME  from " + U8DataAcc.DataBase + ".DistrictClass";
            tbRegion.BaoTitleNames = "cDCCODE=地区编码 ,CDCNAME=地区名称";
            this.tbRegion.FrmHigth = 600;
            this.tbRegion.FrmWidth = 400;
            tbRegion.DecideSql = "select cDCCODE,CDCNAME  from " + U8DataAcc.DataBase + ".DistrictClass where CDCNAME=";

...
namespace FormMain.LoginBAL
{
    public class LoginBL
    {
        ...
        {
            string sql = string.Format("select AutoUserID,UserId,UserName,[Password],a.AuditGroupId,a.DeptName,case State when 1 then '正常' else '已封存' end as State,Memo," +
        ...
        {
            string sql = string.Format("select AutoUserID,UserId,UserName,[Password],a.AuditGroupId,a.DeptName, case State when 1 then '正常' else '已封存' end as State,Memo,b.AuditGroupId,b.AuditGroupName " +
                " from [Users]  "
                + "a left outer join [AuditGroup] b  on a.AuditGroupId=b.AuditGroupId "
                +"where  UserId!='demo' and  Memo !='系统管理员'   union       "
          + "select AutoUserID,UserId,UserName,[Password], case State when 1 then '正常' else '已封存' end as State,Memo,b.AuditGroupId,b.AuditGroupName "+
                     " from [Users] "
                     + "a left outer join [AuditGroup] b  on a.AuditGroupId=b.AuditGroupId "
        ...
        public void Insert_SQL_Users(string UserId,string UserName,string Password,string Memo,string AuditGroupId,string cDepName)
        {
            string sql = string.Format("insert [Users](UserId,UserName,[Password],State,Memo,AuditGroupId,DeptName)  "
                                       + "select '{0}','{1}','{2}','1','{3}','{4}','{5}'", UserId, UserName, Password, Memo, AuditGroupId,cDepName);
        ...
        /// <param name="Memo">备注</param>
        public void Update_SQL_Users(string user, string oldPwd, string newPwd, string UserName, string Memo, string AuditGroupId, string DeptName)
        {
            string sql = string.Format("update  [Users] set [Password]='{2}',UserName='{3}',Memo='{4}',AuditGroupId='{5}',DeptName = '{6}' where UserId='{0}' and [Password]='{1}'  ", user, oldPwd, newPwd, UserName, Memo, AuditGroupId,DeptName);
            Bao.DataAccess.DataAcc.ExecuteNotQuery(sql);
        }

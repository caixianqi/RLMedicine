...
namespace Install
{
    public partial class RPTsearch : Form
    {
        ...
        public static DateTime dtcheckstart;
        public static DateTime dtcheckend;
        public static bool state;
        public static string str_MakeCode;
        public RPTsearch()
        {
            InitializeComponent();
            ...
            dtcheckstart = DateTime.Now.AddMonths(-1);
            dtstart = dtpS.Value.Date;
            dtend = dtpE.Value.Date;
            dtcheckstart = dtCS.Value.Date;
            dtcheckend = dtCE.Value.Date;
            chbS.Checked = true;
            chbE.Checked = false;
            chbS1 = chbS.Checked;
            chbE1 = chbE.Checked;
        ...
        private void RPTsearch_Load(object sender, EventArgs e)
        {
            ...
            aAccStd.BaoSQL = "select distinct aAccCode,aAccStd,aAccName from InsAccessory";
            aAccStd.BaoTitleNames = "aAccCode=编码 ,aAccStd=规格型号 ,aAccName=配件名称";
            this.aAccStd.FrmHigth = 300;
            this.aAccStd.FrmWidth = 500;
            aAccStd.DecideSql = "select distinct aAccCode,aAccStd,aAccName from InsAccessory where aAccStd=";
            aAccStd.BaoColumnsWidth = "aAccCode=150,aAccStd=150,aAccName=250";
            aAccStd.BaoClickClose = true;

            this.dtpS.Value = DateTime.Now.AddMonths(-1);
        }
        ...
        private void Removed_Click(object sender, EventArgs e)
        {
            ...
            int length = aAccStdNames.Length;
            aAccStdNames[i] = aAccStdNames[length - 1];
            Array.Resize(ref aAccStdNames, length - 1);
        ...
        private void dtCS_ValueChanged(object sender, EventArgs e)
        {
            dtcheckstart = dtCS.Value.Date;
        }

        private void dtCE_ValueChanged(object sender, EventArgs e)
        {
            dtcheckend = dtCE.Value.Date;
        }
        
        private void MakeCode_tb_TextChanged(object sender, EventArgs e)
        {
            str_MakeCode = MakeCode_tb.Text;
        }
    }
}

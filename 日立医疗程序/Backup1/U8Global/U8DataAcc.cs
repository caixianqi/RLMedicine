...
namespace U8Global
{
    public class U8DataAcc
    {
        ...
        /// 登陆界面的业务日期
        /// </summary>
        public static DateTime U8_CurDate;//登陆界面的业务日期
        /// <summary>
        /// 工作站的唯一序列号
        /// </summary>
        ...
        /// U8数据库名称
        /// </summary>
        public static string U8DataBase; //U8数据库名称
        ...
        /// U8服务器.数据库
        /// </summary>
        public static string U8ServerAndDataBase;  //数据库名称
        ...
        public static void U8GetConn()
        {
            ...
            if (U8MainConn.State == System.Data.ConnectionState.Closed)
            {
                if (DefConnStr != null || DefConnStr != "")
                {
                    ...
                    U8MainConn.ConnectionString = DefConnStr;
                }
                try
                {
                    U8MainConn.Open();
        ...
        /// <returns></returns>
        /// 
        public static DataSet U8GetDataSet(string sqlStr)
        {
        ...
        /// <returns></returns>
        public static DataSet U8GetDataSet(SqlCommand sqlcmd)
        {
            SqlConnection cn = null;
            try
            {
        ...
        #region  打开连接   GetConn()
        /// <summary>
        ///  (Lin 2020-07-01 修改)新修改获取对应该[U13SHOUHOU].dbo.RL_DBInfo的账套信息
        /// </summary>
        /// <param name="AccountNum"></param>
        /// <returns></returns>
        public static DataTable Get_RL_DBInfo(string AccountNum)
        {
            MD5 md5 = new MD5();
            ...
            string rilidatabase = md5.Md5Decrypt(IniObj.read("database", "database", ""));
            ...
            conn = "uid=" + user + ";initial catalog=" + rilidatabase + ";data source=" + server + ";password=" + password + ";Connect Timeout=10000";

            using (var localConn = new SqlConnection(conn))
            {
                ...
                string strSql = string.Format(@"select * from [U13SHOUHOU].dbo.RL_DBInfo where AccountNum='{0}'", AccountNum);
                var da = new SqlDataAdapter(strSql, localConn);
                var ds = new DataSet();
                da.Fill(ds);
                return ds.Tables[0];
            }
        ...
        /// </summary>
        /// <param name="AccountNum"></param>
        public static void GetOpenConn(string AccountNum)
        {
            MainConn.Close();
            if (MainConn.State == System.Data.ConnectionState.Closed)
            {
                ...
                DataTable dt = Get_RL_DBInfo(AccountNum);
                if (dt != null && dt.Rows.Count > 0)
                {
                    //Server,DataBase,User,PassWord
                    server = dt.Rows[0]["Server"].ToString().Trim();
                    rilidatabase = dt.Rows[0]["DataBase"].ToString().Trim();
                    user = dt.Rows[0]["User"].ToString().Trim();
                    password = md5.Md5Decrypt(dt.Rows[0]["PassWord"].ToString().Trim());

                    U8DataBase = dt.Rows[0]["U8DataBase"].ToString().Trim();
                    U8ServerAndDataBase = "[" + dt.Rows[0]["U8Server"].ToString().Trim() + "]" + ".[" + dt.Rows[0]["U8DataBase"].ToString().Trim() + "].dbo";
                }
       ...
       /// </summary>
       /// <param name="AccountNum"></param>
        public static void GetConn(string AccountNum)
        {
            ...
            if (MainConn.State == System.Data.ConnectionState.Closed)
            {
               ...
               /* if (!System.IO.File.Exists(Application.ExecutablePath.Remove(Application.ExecutablePath.LastIndexOf(@"\") + 1) + "RiliDBini.ini"))
                ...
                string rilidatabase = IniObj.read("database", "database", "");

                //2016-06-29 如果有传入，则不用读取配置文件 tdh
                if (string.IsNullOrEmpty(_u8database))
                {
                    U8DataBase = IniObj.read("database", "u8database", "");
                }
                else
                {
                    U8DataBase = _u8database;
                }

                string u8server = "[" + IniObj.read("database", "u8server", "") + "]";

                U8ServerAndDataBase = u8server + ".[" + U8DataBase + "].dbo";
        ...
        /// 打开连接   GetConn()
        /// </summary>
     /*   public static void GetConn(string _u8database)
        {
        ...
        /// <param name="_tran"></param>
        /// <returns>数据更新适配器对象</returns>
        public static SqlDataAdapter U8BuildDataAdpter(SqlCommand _insertCommand, SqlCommand _updateCommand, SqlCommand _deleteCommand, SqlTransaction _tran)
        {
            SqlDataAdapter _da = new SqlDataAdapter();
            _da.InsertCommand = _insertCommand;
            _da.InsertCommand.CommandTimeout = 3600;//设置超时为1小时
        ...
        /// <param name="selectCmd">SQL语句</param>
        /// <returns>数据检索适配器</returns>
        public static SqlDataAdapter U8BuildDataAdpter(string selectCmd)
        {
            //确认打开连接
            U8GetConn();
            SqlDataAdapter _da = new SqlDataAdapter(selectCmd, U8MainConn);
            _da.SelectCommand.CommandTimeout = 3600;//设置超时为1小时
            return _da;
        }
        ...
        /// <param name="cmdText">SQL语句</param>
        /// <returns>数据查询命令</returns>
        public static SqlCommand U8BuildCommand(string cmdText)
        {
            U8GetConn();
            UpLoadCmd.CommandTimeout = 3600;
            return UpLoadCmd;
        ...
        /// <param name="tran">事务对象</param> 
        /// <returns>数据查询命令</returns>
        public static SqlCommand U8BuildCommand(string cmdText, System.Data.SqlClient.SqlTransaction tran)
        {
            U8GetConn();
            UpLoadCmd.CommandTimeout = 3600;
            UpLoadCmd.Connection = tran.Connection;
        ...
        /// <param name="prams">存储过程所需参数</param>
        /// <returns>返回SqlCommand对象</returns>
        private static SqlCommand U8CreateCommand(string cmdText, SqlParameter[] prams)
        {
            // 确认打开连接
            U8GetConn();
            UpLoadCmd.CommandType = CommandType.StoredProcedure;
        ...
        public static SqlCommand U8CreateCommand(string cmdText)
        {
            U8GetConn();
            UpLoadCmd.CommandType = CommandType.StoredProcedure;
            return UpLoadCmd;
        ...
        public static bool CopyAndOpenExcel(Excel.Application _myExcel, string _templateName)
        {
            ...
            {
                //string U8Soft="D:\\U872SOFT\\UAP\\Runtime\\ZT";
                filename = mySave.FileName;
                //FileInfo mode = new FileInfo(_templatePath);
                //绝对路径
                //FileInfo mode = new FileInfo("D:\\U872SOFT\\UAP\\Runtime\\ZT\\" + _templateName + ".xls");
                //相对路径
...
//                                                            , acc_Id,iYear,userId,cAuth_id);
            string sqlstr = string.Format(@"declare @count1 decimal(18,0)
                                               ...
                                               acc_Id, iYear, userId, cAuth_id);
            string temp = U8ExecuteScalar(sqlstr);
            return temp.Trim() != "0";
        }
        ...
        {
            return GetHoldAuth(U8DataAcc.AccID, U8DataAcc.iYear, U8DataAcc.cUserId, cAuth_Id);
        }

    ...
    public class BillBill :Bao.BillBase.BillBase
    {
        ...
        public override void UnAuditBefor()
        {
            ...
            if (UFBaseLib.BusLib.BaseInfo.DBUserID == dt.Rows[0]["AuditerCode"].ToString())
            {
                ...
                {
                    throw new Exception("寄票状态已经提交，不能弃审");
                }
                string strstatus = RiLiGlobal.RiLiDataAcc.RiLiExecuteScalar(string.Format(@"select b.AuditPerson from price a
                                      inner join FaultFeedback b on a.RepairMissionID = b.RepairMissionID
                                      where PriceId = '{0}'", dt.Rows[0]["priceid"].ToString()));

                if (strstatus.Length > 0)
                {
                    throw new Exception("维修反馈已审核，不能弃审");
                }
